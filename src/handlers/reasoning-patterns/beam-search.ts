...
import {
  ...
  ConvergenceCriteria,
  PathComparison,
  BeamScoringComponents
} from '../../types/reasoning-patterns/beam-search.js';
import { ThoughtData } from '../../types/index.js';

export class BeamSearchHandler implements BeamSearchOperations {
  private readonly defaultConfig: BeamSearchConfig = {
  ...
  private readonly defaultConvergenceCriteria: ConvergenceCriteria = {
    maxGenerations: 10,
   ...
   */
  private calculatePathScore(path: BeamSearchPath, session: BeamSearchSession): number {
    const components: BeamScoringComponents = {
      baseScore: 0,
      finalScore: 0
    };
  ...
  private calculatePathDiversity(path: BeamSearchPath, session: BeamSearchSession): number {
    if (session.activePaths.length <= 1) return 1;
   ...
   */
  private calculatePathSimilarity(path1: BeamSearchPath, path2: BeamSearchPath): number {
  ...
  private calculateConsensus(session: BeamSearchSession): number {
    ...
    const paths = session.activePaths.map(id => session.paths.get(id)).filter(Boolean) as BeamSearchPath[];
   ...
   */
  private calculateScoreImprovement(session: BeamSearchSession): number {
    ...
    if (recentScores.length < 2) return 1;
  ...
  mergePaths(pathIds: string[], session: BeamSearchSession): BeamSearchPath {
    ...
    const paths = pathIds.map(id => session.paths.get(id)).filter(Boolean) as BeamSearchPath[];
    ...
    const mergedPath: BeamSearchPath = {
      id: mergedPathId,
   ...
   */
  private mergeNodeSequences(sequences: string[][]): string[] {
  ...
  getBestPath(session: BeamSearchSession): BeamSearchPath | null {
    if (session.paths.size === 0) return null;
    
    let bestPath: BeamSearchPath | null = null;
  ...
  calculateDiversity(session: BeamSearchSession): number {
    if (session.activePaths.length <= 1) return 0;
   ...
   */
  private attemptPathMerging(session: BeamSearchSession): void {
    if (!session.config.mergeThreshold) return;
    
    const paths = session.activePaths.map(id => session.paths.get(id)).filter(Boolean) as BeamSearchPath[];
    const merged = new Set<string>();
    
    for (let i = 0; i < paths.length; i++) {
      if (merged.has(paths[i].id)) continue;
      
      const similarPaths: string[] = [paths[i].id];
      
      for (let j = i + 1; j < paths.length; j++) {
        if (merged.has(paths[j].id)) continue;
   ...
   */
  private updateStatistics(session: BeamSearchSession): void {
